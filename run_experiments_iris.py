...
import bilevel_captrade_bisection_and_lagrangian as bct
import json
import pandas as pd
import random
...
params['algotype'] = 'bisection'
params["folder_input"] = "instances_IB"  
params["folder_output"] = "results_IB_IRIS"
params['maxIterations'] = 30
params['maxtime'] = 100 # max time for each gurobi call
params["maxtimetotal"] = 600 # total running time for each instance
 
instance_file = 'instances_bilevel_IB.csv'
#instance_file =  #'instances_bilevel_IR_bisection.csv'
#id = 'I_0V'
#deltas = [0.7, 0.75, 0.80, 0.85, 0.90]
df = pd.read_csv(instance_file, index_col='id', dtype={'himprov': float})
if RESET:
    df['lobjval'] = -1.0
    df['lz1'] = -1.0
    df['lz2'] = -1.0
...
for index, row in df.iterrows():
        id = index
        if DEBUG:
               id = "IR_B3000_D250_F12_L4_T_5_seed_0"
               index = "IR_B3000_D250_F12_L4_T_5_seed_0"
        #for delta in deltas:
        delta = row['delta']
        ...
        if row['solved'] and not DEBUG:
                continue
        print(f"running instance {id} with delta {delta}")

        result = bct.run(params)
        ...
        df.at[index, 'himprov'] = round(result['improvementHeuristic'],2)
